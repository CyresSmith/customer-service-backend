...
import { IBasicUserInfo, IBasicUserInfoWithTokensAndCompanies } from 'src/users/users.types';
...
export class AuthController {
    ...
    @Get('refresh')
    @HttpCode(200)
    async refresh(@Request() { user }: { user: IBasicUserInfo }): Promise<ITokenPair> {
        const tokenPair = await this.tokenService.generateNewTokenPair(user);
        await this.usersService.updateTokens(user.id, tokenPair);
    ...
    @Post('logout')
    @HttpCode(200)
    async logout(@Request() { user }: { user: IBasicUserInfo }): Promise<MessageResponse> {
